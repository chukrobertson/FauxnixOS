...
PERSON_TAG_SOURCE = "person"


IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp", ".tif", ".tiff", ".heic", ".heif", ".dng", ".raw"}
VIDEO_EXTS = {".mp4", ".mov", ".mkv", ".avi", ".webm", ".m4v", ".wmv", ".mpg", ".mpeg", ".3gp", ".mts", ".m2ts"}


KEYWORD_TAGS = [
    ("screenshot", {"screenshot", "screen shot", "screen_capture", "screen-capture"}),
...
def suggested_auto_tags(record: dict | None, *, face_count: int = 0, extra_tags: list[str] | None = None) -> list[str]:
    ...
    if category == "image" or ext in IMAGE_EXTS:
        tags.update({"image", "picture"})
        if ext in {".jpg", ".jpeg", ".heic", ".heif", ".dng", ".raw"}:
            tags.add("photo")
    if category == "video" or ext in VIDEO_EXTS:
        tags.update({"video", "moving image"})
    if extracted and not extracted.startswith("[OCR extraction error]"):
        tags.add("has text")
    if face_count > 0:
        tags.update({"has faces", "face review"})
...
def person_tag_name(display_name: str) -> str:
    return clean_tag_name(f"person: {display_name}")


def apply_person_tag_to_files(person_id: int, file_ids: list[int], *, source: str = PERSON_TAG_SOURCE) -> dict:
    unique_file_ids = sorted({int(file_id) for file_id in file_ids if file_id})
    if not unique_file_ids:
        return {"applied": 0, "files": [], "reason": "no_files"}

    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT display_name FROM people WHERE id = ?", (int(person_id),))
    row = cur.fetchone()
    if not row:
        conn.close()
        return {"applied": 0, "files": unique_file_ids, "reason": "missing_person"}

    tag = person_tag_name(row["display_name"])
    tag_id = _ensure_tag(cur, tag)
    ts = time.time()
    applied = 0
    for file_id in unique_file_ids:
        ...
        cur.execute(
            "INSERT OR IGNORE INTO file_tags (file_id, tag_id, source, created_ts) VALUES (?, ?, ?, ?)",
            (file_id, tag_id, source, ts),
        )
        applied += cur.rowcount
    conn.commit()
    conn.close()
    return {"tag": tag, "tag_id": tag_id, "applied": applied, "files": unique_file_ids}
