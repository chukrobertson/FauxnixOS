"""Test input injection via XTEST into an Xvfb-hosted xterm."""
...
def main():
    marker = "/tmp/fauxnix-xvfb-marker"
    ...
    print("process tree before input:")
    subprocess.run(["ps", "-ef", "--forest"], check=False)

    dpy = display.Display(disp_str)
    # Load US keymap so keycodes map to expected keysyms.
    subprocess.run(
        ["setxkbmap", "-display", disp_str, "us"],
        check=False,
        timeout=5,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    # Capture frame
    raw = root.get_image(0, 0, 400, 300, X.ZPixmap, 0xFFFFFFFF)
    data = raw.data
    ppm = "/tmp/fauxnix-xvfb.ppm"
    ...
    if os.path.exists(marker):
        print("SUCCESS - Xvfb XTEST marker created")
    else:
        print("FAILURE - Xvfb XTEST marker not created")
