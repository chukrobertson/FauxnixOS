"""Type into xterm running bash -i explicitly."""
...
def main():
    marker = "/tmp/fauxnix-bash-marker"
    if os.path.exists(marker):
        os.remove(marker)

    p = XwaylandPerApp(argv=["xterm", "-e", "bash", "-i"], width=400, height=300)
    p.start()
    time.sleep(4)
    ...
    if os.path.exists(marker):
        print("SUCCESS - bash -i accepted typed command")
    else:
